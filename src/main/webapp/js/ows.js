...
function addCopyPasteFunctionality_old(){
	dojo.query("td.select, th.select, span.select, div.select, li.select").onclick(function(e){
		var editE=e.target;
...
function correcturSelectElement(editE){
	console.log("moveMenuItems 3 "+editE);
	for ( var i = 0; i < 4; i++) {
		console.log("moveMenuItems 3 1 ");
		if(dojo.hasClass(editE,"select")){
			break;
		}
		editE=editE.parentNode;
	}
	console.log("moveMenuItems 4 "+editE);
	return editE;
}
...
function moveMenuItems(){
	...
	dojo.query(".select").connect("oncontextmenu", function(e) {
		console.log("moveMenuItems 2 ");
		console.log("moveMenuItems 2 "+e);
